...
sealed class ResultState<out T> {
    companion object {
        ...
        fun <T> onAppLoading(loadingMessage: String)= Loading(loadingMessage)
...
fun <T> MutableLiveData<ResultState<T>>.paresResult(result: BaseResponse<T>) {
    value = when {
        result.isSuccess() -> {
            ...
            ResultState.onAppError(AppException(result.getResponseCode(), result.getResponseMessage()))
        }
    }
}
...
fun <T> MutableLiveData<ResultState<T>>.paresException(e: Throwable) {
    this.value = ResultState.onAppError(ExceptionHandle.handlerException(e))
}
